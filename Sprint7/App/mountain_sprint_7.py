...
class mainWindow(qtw.QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(qtg.QIcon('Data/mountains.png'))

        # Sprint-4 Step-4
        # Create a QComboBox called comboBox, that contains
        # the input file names.
        self.comboBox = qtw.QComboBox(self)
        self.comboBox.addItems(['Small', 'Medium', 'Large', 'Trivial'])
        ...
        self.label1.setPixmap(qtg.QPixmap('Data/mountains.png'))
        ...
        self.label2.setStyleSheet("QLabel { color : red; font: bold;}")
        self.label2.hide()
        ...
        self.loadCanvas_btn = qtw.QPushButton(
            'Display Canvas', clicked=self.loadCanvas)
        ...
        self.animate_btn = qtw.QPushButton(
            'Animate Flow', clicked=self.animate)
        ...
        vlayout.addWidget(self.loadDb_btn)
        vlayout.addWidget(self.loadCanvas_btn)
        vlayout.addWidget(self.animate_btn)
        vlayout.addStretch()
    ...
    def loadDb(self):
        ...
        current_file_location = Path(__file__).absolute().parent
        path = str(current_file_location) + '/../Data/' + \
            comboText.lower() + '.txt'
        file = open(path, 'r')
        ...
        line1_extract = lines[0].split(" ")
        self.n_rows = int(line1_extract[0])
        self.n_columns = int(line1_extract[1])
        ...
        line2_data = []
        for item in line2_extract:
            line2_data.append(float(item))
        ...
        max_elevation = max(line2_data)
        min_elevation = min(line2_data)
        diff_elevation = max_elevation - min_elevation

        # Sprint-5 Step-9
        # Create a connection (called connection) to your MountainDB
        # change this to your server name (look on SSMS)
        server = 'LAPTOP-CH2AA0JL'
        database = 'MountainDb'
        ...
        query = "SELECT File_id FROM MountainDb.dbo.Files WHERE File_name = '" + comboText + "'"
        ...
        if file_record is None:
            try:
                ...
                params = [comboText, self.n_rows, self.n_columns]
                ...
                for i in range(0, len(line2_data)):
                    ...
                    elevation = line2_data[i]
                    # Compile the data point's color
                    color_value = int(
                        (elevation - min_elevation) / (diff_elevation) * 255)
                    ...
                    params = [elevation, color, File_id]
                    cursor.execute(query, params)
    ...
    def loadCanvas(self):
        ...
        # value as comboText
        cursor = connection.cursor()
        query = "SELECT * FROM MountainDb.dbo.Files WHERE File_name = '"+comboText + "'"
        cursor.execute(query)
        file_record = cursor.fetchone()
        ...
        query = "SELECT DataPoint_id, DataPoint_elevation, DataPoint_base_color FROM MountainDb.dbo.DataPoints WHERE File_id =" + \
            str(file_id) + " ORDER BY DataPoint_id"
        cursor.execute(query)
        ...
        self.color = " "
        x_Index = 0
        for x in self.grid_data:
            y_index = 0
            for y in x:
                # Plot the point by extracting its base_color value
                # from the object stored in self.grid_data
                self.color = y.base_color
                pen.setColor(qtg.QColor(
                    self.color))
                painter.setPen(pen)
                painter.drawPoint(x_Index, y_index)
                y_index += 1
            x_Index += 1
    ...
    # borders: 0 -> self.n_columns and 0 -> self.n_rows)
    def mousePressEvent(self, event):
        if event.button() == qtc.Qt.LeftButton:
            ...
            if x1 < 0:
                x1 = 10
            elif x2 > self.n_columns:
                x2 = self.n_columns-10
    ...
    def update_data_point_color(self, x, y):
        ...
        water_level = self.grid_data[x][y].water_level
        painter = qtg.QPainter(self.label1.pixmap())
        pen = qtg.QPen()
        if water_level < 1:
            self.color = self.grid_data[x][y].base_color
            pen.setColor(qtg.QColor(
                self.color))
            painter.setPen(pen)
    ...
    def animate(self):
        ...
        while self.continue_animating == True:
            ...
            water_copy = []
            for key in self.water_data.keys():
            ...
            for key in water_copy:
                ...
                lowKey = str(low_x) + "/" + str(low_y)
                lowValue = ''
                self.water_data[lowKey] = lowValue
                # Sprint-7 Step-20
        ...
        for x in self.grid_data[:10]:
            ...
            for i in x:
                if i.water_level > 0:
                    i.water_level = 0
                    try:
                        key = list(self.water_data.keys())[index_Key]
                        del self.water_data[key]
                        self.update_data_point_color(index_X, index_I)
                    except IndexError:
                        pass
    ...
    def find_lowest_neighbour(self, x, y):
        ...
        #                   and coordinates
        lowest_height = self.grid_data[x][y].elevation + \
            self.grid_data[x][y].water_level
        ...
        if x-1 >= 0 and y-1 >= 0:
            ...
            top_left_Neighbour = self.grid_data[x - 1][y - 1].elevation + \
                self.grid_data[x - 1][y - 1].water_level

            if lowest_height > top_left_Neighbour:
                low_x -= 1
                low_y -= 1
                lowest_height = self.grid_data[low_x][low_y].elevation + \
                    self.grid_data[low_x][low_y].water_level

        # Sprint-7 Step-30: Test neighbour - Top

        if x-1 >= 0:

            top_Neighbour = self.grid_data[x - 1][y].elevation + \
                self.grid_data[x - 1][y].water_level

            if lowest_height > top_Neighbour:
                low_x -= 1
                lowest_height = self.grid_data[low_x][low_y].elevation + \
                    self.grid_data[low_x][low_y].water_level
        # Sprint-7 Step-31: Test neighbour - Top Right

        if x-1 >= 0 and y+1 <= self.n_rows - 1:
            top_right_Neighbour = self.grid_data[x - 1][y + 1].elevation + \
                self.grid_data[x - 1][y + 1].water_level

            if lowest_height > top_right_Neighbour:
                low_x -= 1
                low_y += 1
                lowest_height = self.grid_data[low_x][low_y].elevation + \
                    self.grid_data[low_x][low_y].water_level
        ...
        if y-1 >= 0:
            left_Neighbour = self.grid_data[x][y - 1].elevation + \
                self.grid_data[x][y - 1].water_level

            if lowest_height > left_Neighbour:
                low_y -= 1
                lowest_height = self.grid_data[low_x][low_y].elevation + \
                    self.grid_data[low_x][low_y].water_level
        # Sprint-7 Step-33: Test neighbour - Right
        if y+1 <= self.n_rows - 1:
            right_Neighbour = self.grid_data[x][y + 1].elevation + \
                self.grid_data[x][y + 1].water_level

            if lowest_height > right_Neighbour:
                low_y += 1
                lowest_height = self.grid_data[low_x][low_y].elevation + \
                    self.grid_data[low_x][low_y].water_level
        # Sprint-7 Step-34: Test neighbour - Left bottom
        if x+1 <= self.n_columns - 1 and y-1 >= 0:
            left_bottom_Neighbour = self.grid_data[x + 1][y - 1].elevation + \
                self.grid_data[x + 1][y - 1].water_level

            if lowest_height > left_bottom_Neighbour:
                low_x += 1
                low_y -= 1
                lowest_height = self.grid_data[low_x][low_y].elevation + \
                    self.grid_data[low_x][low_y].water_level
        # Sprint-7 Step-35: Test neighbour - Bottom
        if x+1 <= self.n_columns - 1:
            bottom_Neighbour = self.grid_data[x + 1][y].elevation + \
                self.grid_data[x + 1][y].water_level

            if lowest_height > bottom_Neighbour:
                low_x += 1
                lowest_height = self.grid_data[low_x][low_y].elevation + \
                    self.grid_data[low_x][low_y].water_level
        # Sprint-7 Step-36:  Test neighbour - Right bottom
        if x+1 <= self.n_columns - 1 and y+1 <= self.n_rows - 1:
            rigth_bottom_Neighbour = self.grid_data[x + 1][y + 1].elevation + \
                self.grid_data[x + 1][y + 1].water_level

            if lowest_height > rigth_bottom_Neighbour:
                low_x += 1
                low_y += 1
                lowest_height = self.grid_data[low_x][low_y].elevation + \
                    self.grid_data[low_x][low_y].water_level
...
class DataPoint():
    def __init__(self, elevation, color):
        # Must be float to cater for data format
        self.elevation = float(elevation)
        self.base_color = color
        # Sprint-6 Step-2
        # Make it a float to simplify calculations that include the elevation
